...
class BunkRecommendationEngine {
  ...
  BunkPlan generatePlan(Timetable timetable, AcademicCalendar calendar) {
    ...
    // 3. Initialize Subject Status Map (Calculate 25% max bunks for exactly 75% attendance)
    ...
    for (final s in totalClassesPerSubject.keys) {
      ...
      // Truncate to ensure we never drop below 75%
      final maxBunks = (total * 0.25).floor();
